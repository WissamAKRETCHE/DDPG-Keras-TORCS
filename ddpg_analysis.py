    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            if analysis:
                ...
                for output in list(last_layer_model.predict(s_t.reshape(1, s_t.shape[0]))[0]):
                    activationFiles[j].write(str(output)+"\n") 
            ...
            if analysis and step == 2000:
                env.end()  # This is for shutting down TORCS
                print("Finish.")
                for i in range(len(activationFiles)):
                    activationFiles[i].close()
                return
            step += 1
            if done:
                break
